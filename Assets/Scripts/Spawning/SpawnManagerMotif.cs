...
namespace MRMotifs.SharedActivities.Spawning
{
    ...
    public class SpawnManagerMotif : NetworkBehaviour
    {
        ...
        [SerializeField]
        private bool m_useOpenPlayArea = false;
        ...
        {
            // With open play area mode, spawn locations are not used
            // Players spawn at their physical headset position
            // Just initialize arrays to empty/default values
        ...
        {
            // In open play area mode, skip spawn point teleportation - players stay where they physically are
            // This is ideal for colocation where players share the same physical space
            if (m_useOpenPlayArea)
            {
                RequestRemovePlayerFromQueueRpc(player);
                Debug.Log("[SpawnManagerMotif] Open play area mode - player spawns at current location");
                yield break;
            }
