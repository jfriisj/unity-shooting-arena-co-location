...
Technical performance metrics are collected through the MetricsLogger system at 1Hz sampling rate:
\begin{itemize}
    \item \textbf{Frame Rate:} Averaged FPS for performance assessment
    \item \textbf{Network Latency:} RTT monitoring via Photon Fusion (threshold: $\leq75$ms)
    \item \textbf{Calibration Error:} Spatial anchor alignment deviation in millimeters (threshold: $<10$mm)
    \item \textbf{Participant Count:} Number of connected headsets
    \item \textbf{Device Status:} Battery level and temperature for thermal throttling detection
\end{itemize}
